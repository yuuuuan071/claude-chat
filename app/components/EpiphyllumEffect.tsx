...
export default function EpiphyllumEffect({ onAnimationComplete, onScatterComplete, triggerScatter }: EpiphyllumProps) {
  ...
  useEffect(() => {
    ...
    function generateCurvedPetal(
      baseAngle: number, petalLen: number, petalWid: number,
      startR: number, startH: number, endH: number,
      inwardCurl: number, count: number
    ): number[] {
      const pts: number[] = [];
      for (let i = 0; i < count; i++) {
        ...
        pts.push(Math.cos(angle) * actualR, h + (Math.random() - 0.5) * 0.008, Math.sin(angle) * actualR);
      }
      return pts;
    }
