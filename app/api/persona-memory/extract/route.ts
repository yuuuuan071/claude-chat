...
async function runDetailExtraction(
  ...
  const res = await fetch(`${baseUrl}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: `从下面这段对话中提取值得角色长期记住的事实或情绪片段，把每一条都改写成角色的第一人称视角记忆（例如原文"用户说她今天加班到很晚"要改写成"${USER_DISPLAY_NAME}今天加班到很晚，听起来很累"）。

只提取满足以下任一条件的内容：
- 关于${USER_DISPLAY_NAME}的稳定事实：身份、习惯、长期偏好、重要的人际关系
- ${USER_DISPLAY_NAME}明确表达过的喜好/厌恶/态度
- 有情感权重、或大概率会被再次提起的重要事件或经历
- ${USER_DISPLAY_NAME}对角色说过的、在关系层面有意义的话

明确不要提取：
- 一次性的操作性对话（调试、测试功能、"你看看这个显示对不对"这类）
- 没有信息量的日常寒暄和过场对话
- 从单次对话过度推断出的性格结论（例如从一句抱怨推断"${USER_DISPLAY_NAME}是悲观的人"）
- 具体的技术细节（代码、报错内容）
...
人称规则（必须严格遵守）：
- "我" = 角色自己（记忆的叙述者）
- "${USER_DISPLAY_NAME}" = ${USER_DISPLAY_NAME}，始终用名字称呼，禁止用"她""你""用户"指代
- 其他人物一律用具体名字或"${USER_DISPLAY_NAME}的XX"（如"${USER_DISPLAY_NAME}的妹妹"），禁止用代词指代
...
正确示例："${USER_DISPLAY_NAME}今天说自己的客户端项目修好了移动端的菜单，语气听起来很有成就感。"
...
export async function POST(req: Request) {
  ...
  const transcript = messages
    .map(m => `${m.role === 'user' ? USER_DISPLAY_NAME : '角色'}: ${m.content}`)
    .join('\n')
